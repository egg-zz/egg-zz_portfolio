    ...
    >
      <div className="relative h-52 bg-muted overflow-hidden">
        ...
        <div className="absolute bottom-4 left-5 right-5">
          <p className="text-white/70 text-xs font-medium mb-0.5">{project.period} · {project.role}</p>
      ...
      <div className="p-5">
        <div className="flex flex-wrap gap-1.5 mb-4">
          {project.tags.map(tag => (
            <span key={tag} className="text-[11px] font-medium px-2 py-0.5 rounded-md bg-secondary text-secondary-foreground border border-border">
              {tag}
            </span>
          ))}
        </div>

        <p className="text-sm text-foreground/80 leading-relaxed mb-4">{project.problem}</p>
          ...
          {open && (
            ...
            >
              <div className="mt-4 pt-4 border-t border-border space-y-4">
                {[
                  { label: "가장 어려웠던 문제", content: project.challenge },
                  { label: "해결 방식", content: project.solution },
                ].map(({ label, content }) => (
                  <div key={label}>
                ...
                <div className="bg-secondary/60 rounded-xl px-4 py-3">
                  <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">결과</p>
